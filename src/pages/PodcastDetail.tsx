...
import { getPodcastById, getImg, getAudioUrl } from '@/lib/api';
...
const PodcastDetail = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    getPodcastById(id)
      .then(res => setPodcast(res?.data || null))
      .catch(() => {})
  ...
  return (
    <div className="p-4 pb-40">
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-sm font-bold text-foreground truncate">{podcast.name || 'Podcast'}</span>
      </div>
      ...
      <div className="flex gap-4 items-end mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <h2 className="text-lg font-black text-foreground leading-tight mt-1">{podcast.name}</h2>
          <p className="text-xs text-muted-foreground mt-1">{podcast.totalEpisodes || episodes.length} episodes</p>
          {podcast.description && (
            <p className="text-[11px] text-muted-foreground/60 mt-1 line-clamp-2">{podcast.description}</p>
          )}
        </div>
      </div>
        ...
        {episodes.map((ep: any, i: number) => {
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-bold text-foreground truncate">{ep.name || ep.title || `Episode ${i + 1}`}</p>
                {ep.description && <p className="text-[11px] text-muted-foreground/60 truncate mt-0.5">{ep.description}</p>}
                {ep.duration && (
