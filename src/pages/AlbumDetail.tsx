...
import { ArrowLeft, Play, Share2, Loader2 } from 'lucide-react';
...
const AlbumDetail = () => {
  ...
  if (!album) {
    return <div className="text-center py-16 text-muted-foreground">Album not found</div>;
  }
  ...
  return (
    <div className="p-4 pb-40">
      ...
      <div className="flex items-center gap-3 mb-4">
        ...
        <span className="text-sm font-semibold text-foreground truncate">{album.name || 'Album'}</span>
      </div>
      ...
      <div className="flex gap-4 items-end mb-5">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[10px] uppercase tracking-wider text-primary font-bold mb-1">Album</p>
          <h2 className="text-lg font-extrabold text-foreground leading-tight">{album.name || 'Unknown Album'}</h2>
      ...
      {songs.length > 0 && (
        <>
          <h3 className="text-sm font-bold text-foreground mb-3 flex items-center gap-2">🎵 Tracks ({songs.length})</h3>
