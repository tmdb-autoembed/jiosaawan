...
const SongItem = ({ song, songList, songIdx = -1, showMeta = true }: SongItemProps) => {
  ...
  return (
    <div
      onClick={handleClick}
      className={`flex items-center gap-3 p-2.5 rounded-lg nm-surface nm-raised cursor-pointer transition-all duration-200 hover:translate-x-0.5 active:nm-inset active:scale-[0.98] ${
    ...
    >
      {imgUrl ? (
        <img
          ...
          alt={song.name || ''}
          loading="lazy"
          className="w-[52px] h-[52px] rounded-xl object-cover flex-shrink-0 nm-flat"
          onError={(e) => { (e.target as HTMLImageElement).src = ''; }}
        />
      ) : (
        <div className="w-[52px] h-[52px] rounded-xl bg-secondary flex-shrink-0 nm-flat" />
      )}
      ...
      <div className="flex-1 min-w-0">
        ...
        {showMeta && metaParts.length > 0 && (
          <p className="text-[11px] text-muted-foreground/60 truncate mt-0.5">{metaParts.join(' • ')}</p>
        )}
      </div>
