      ...
      <div className="bg-background/95 border-t border-border/10 px-3 pt-2.5 pb-2">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0 cursor-pointer" onClick={() => setExpandedOpen(true)}>
            <p className="text-sm font-semibold truncate text-white">{decodeHtml(currentSong.name || currentSong.title || 'Unknown')}</p>
            <p className="text-[11px] truncate" style={{ color: 'hsl(190, 80%, 60%)' }}>{getArtistStr(currentSong)}</p>
          </div>

          {/* Expand button */}
          <button onClick={() => setExpandedOpen(true)} className="p-1.5 text-white/60 hover:text-white transition-colors">
